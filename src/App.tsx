...
export default function App() {
  ...
  const checkSecurity = async () => {
    ...
    try {
      ...
      if (saltValue && authSaltValue && authHashValue) {
        ...
        setAuthSalt(new Uint8Array(base64ToBuffer(authSaltValue)));
        setAuthHash(authHashValue);
        setIsLocked(true);
      ...
      // If we can't read storage, assume we need setup for safety
      setShowMasterSetup(true);
      setIsLocked(false);
    } finally {
  ...
  const checkPasswordStrength = (pass: string) => {
    ...
    if (/[^0-9]/.test(pass)) score += 1; // Bonus for non-numeric if they choose to
  ...
  const handleSetupMaster = async (e: FormEvent) => {
    ...
    if (masterPasswordInput.length < 6) {
      setSecurityError("PIN should be at least 6 digits");
      return;
    }
    ...
    try {
      ...
      if (inputHash !== authHash) {
        ...
        if (newFailed >= 3) {
          ...
          setSecurityError(`Invalid PIN. ${3 - newFailed} attempts remaining.`);
        }
        return;
      }
  ...
  const savePassword = async (e: FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        await storage.updatePassword(editingId, { ...encryptedData, signature });
      } else {
        await storage.savePassword({ ...encryptedData, signature });
      }
  ...
  const saveNote = async (e: FormEvent) => {
    ...
    try {
      ...
      await storage.saveNote({ ...encryptedData, signature });
  ...
  return (
    <div className={cn(
      ...
      <AnimatePresence mode="wait">
        {isOffline && (
          <div className="fixed top-0 left-0 right-0 z-[200] bg-zinc-900 text-white text-[10px] font-bold uppercase tracking-widest py-1 px-4 flex items-center justify-center gap-2">
            <div className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse" />
            Offline Mode: Vault is running locally
          </div>
        )}

        {isInitializing && (
          <div key="loader" className="fixed inset-0 z-[300] bg-zinc-50 flex items-center justify-center">
            <motion.div 
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            >
              ...
              <p className="text-zinc-500 text-sm font-medium">Initializing Secure Environment...</p>
            </motion.div>
          </div>
        )}

        {(!isWindowFocused || !isTabVisible) && !isInitializing && (
          ...
          <div key="master-setup-overlay" className="fixed inset-0 z-[100] bg-zinc-50 flex items-center justify-center p-4">
            ...
            >
              <div className="text-center space-y-2">
                ...
                <p className="text-zinc-500 text-sm">Set a 6-digit PIN to encrypt your vault. This PIN is never stored and cannot be recovered.</p>
              </div>

              <form onSubmit={handleSetupMaster} className="space-y-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Master PIN</label>
                  <input 
                    type="password" 
                    ...
                    onChange={(e) => {
                      ...
                      if (val.length <= 8) { // Allow up to 8 for flexibility but suggest 6
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Confirm PIN</label>
                  <input 
                    type="password" 
                    ...
                    onChange={(e) => setMasterPasswordConfirm(e.target.value.replace(/[^0-9]/g, ''))}
                    required
                  />
                </div>
                {securityError && (
          ...
          <div key="vault-locked-overlay" className="fixed inset-0 z-[100] bg-zinc-50 flex items-center justify-center p-4">
            ...
            >
              <div className="text-center space-y-2">
                ...
                <p className="text-zinc-500 text-sm">Enter your 6-digit PIN to access your data.</p>
              </div>
              ...
              <form onSubmit={handleUnlock} className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <input 
                    ...
                    pattern="[0-9]*"
                    value={masterPasswordInput}
                    onChange={(e) => setMasterPasswordInput(e.target.value.replace(/[^0-9]/g, ''))}
                    required
                    autoFocus
                  />
                </div>
      ...
      <div className={cn(
        ...
        (isCompromised || isLocked || isInitializing) ? "blur-xl scale-[0.98] pointer-events-none select-none" : ""
      )}>
        {isCompromised && (
